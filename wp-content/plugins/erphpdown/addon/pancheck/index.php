...
function epd_custom_post_views($views) {
    if( is_admin() ) {
        ...
        $class_pan = ($_GET['pan_status'] == '0') ? ' class="current"' : '';
        $views['pan_status'] = sprintf(__('<a href="%s"'. $class_pan .'>'. '网盘失效' .' <span class="count">（%d）</span></a>', '' ), admin_url('edit.php?post_status=all&post_type=post&pan_status=0'), $result_pan->found_posts); 
    }
    return $views;
}
