...
if(!is_user_logged_in() && !$start_down2 && !get_option('erphp_wppay_down')){
	wp_die('请先登录网站！',"友情提示");
}
...
if ($postid){
	...
	//if($start_down2 || get_option('erphp_wppay_down')){
	...
	if($ews_erphpdown && function_exists("ews_erphpdown") && isset($_COOKIE['ewd_'.$postid])){
		$hasdown_info = 1;
	}
		...
		if(!$price && $memberDown != 4 && $memberDown != 15 && $memberDown != 8 && $memberDown != 9){
			...
			if($erphp_reg_times_from && $erphp_reg_times_to && !$userType){
				if(date("H") >= $erphp_reg_times_from && date("H") < $erphp_reg_times_to){
					wp_die("用户您好，".$erphp_reg_times_from.":00 — ".$erphp_reg_times_to.":00为网站访问高峰期，免费下载通道拥挤，<a href='".get_option('erphp_url_front_vip')."' target='_blank'>升级VIP</a>，畅想高速下载！","友情提示");
				}
			}
			...
			if(!$userType){
				...
				if($userType == 6 && $erphp_day_times > 0 && $day_times_includes_free){
					if( checkDownLogNoVip($user_info->ID,$postid,$erphp_day_times) ){
						...
						wp_die($erphp_day_name."用户每天只能免费下载".$erphp_day_times."个免费资源！<a href='".constant("erphpdown")."buy.php?postid=".$postid."&index=".$index."'>单独购买</a>","友情提示");
					}
					...
					if( checkDownLogNoVip($user_info->ID,$postid,$erphp_month_times) ){
						...
						wp_die($erphp_month_name."用户每天只能免费下载".$erphp_month_times."个免费资源！<a href='".constant("erphpdown")."buy.php?postid=".$postid."&index=".$index."'>单独购买</a>","友情提示");
					}
					...
					if( checkDownLogNoVip($user_info->ID,$postid,$erphp_quarter_times) ){
						...
						wp_die($erphp_quarter_name."用户每天只能免费下载".$erphp_quarter_times."个免费资源！<a href='".constant("erphpdown")."buy.php?postid=".$postid."&index=".$index."'>单独购买</a>","友情提示");
					}
				}elseif($userType == 9 && $erphp_year_times > 0 && $year_times_includes_free){
					if( checkDownLogNoVip($user_info->ID,$postid,$erphp_year_times) ){

					}else{
						wp_die($erphp_year_name."用户每天只能免费下载".$erphp_year_times."个免费资源！<a href='".constant("erphpdown")."buy.php?postid=".$postid."&index=".$index."'>单独购买</a>","友情提示");
					}
				}elseif($userType == 10 && $erphp_life_times > 0 && $life_times_includes_free){
					if( checkDownLogNoVip($user_info->ID,$postid,$erphp_life_times) ){

					}else{
						wp_die($erphp_life_name."用户每天只能免费下载".$erphp_life_times."个免费资源！<a href='".constant("erphpdown")."buy.php?postid=".$postid."&index=".$index."'>单独购买</a>","友情提示");
					}
				}
			}
