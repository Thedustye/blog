	...
	if($order){
		if(!$order->ice_success){
    		...
    		if($erphp_down == 6){
    			...
    			if(function_exists('doErphpActKa')){
					...
					if($email){
						...
						wp_mail($email, '【'.$ppost->post_title.'】卡密', '您购买的【'.$ppost->post_title.'】卡密：<br>'.$activation_num, $headers);
					}
				}
    		}
			...
			if(get_option('erphp_remind')){
				$headers = 'Content-Type: text/html; charset=' . get_option('blog_charset') . "\n";
				wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】订单提醒 - '.$ppost->post_title, '免登录用户消费'.$total_fee.'元购买了'.$ppost->post_title.get_permalink($post_id), $headers);
			}
...
function epd_set_order_success($order_num,$total_fee,$pay_method=''){
	...
	if($order){
		if(!$order->ice_success){
			...
			if($updatOrder){
				addUserMoney($order->ice_user_id,$total_fee*get_option('ice_proportion_alipay'),'在线充值');
			}
			...
			if($order->ice_post_id){
                        ...
                        if($result)
                        {
				    		...
				    		if($erphp_down == 6){
				    			...
				    			if(function_exists('doErphpActKa')){
									...
									if($email){
										$headers = 'Content-Type: text/html; charset=' . get_option('blog_charset') . "\n";
										wp_mail($email, '【'.$subject.'】卡密', '您购买的【'.$subject.'】卡密：<br>'.$activation_num, $headers);
									}
								}
				    		}else{
	                        	...
	                        	if($down_activation && function_exists('doErphpAct')){
									...
									if($cuser && $cuser->user_email){
										$headers = 'Content-Type: text/html; charset=' . get_option('blog_charset') . "\n";
										wp_mail($cuser->user_email, '【'.$subject.'】激活码', '您购买的资源【'.$subject.'】激活码：'.$activation_num, $headers);
									}
								}
							}
							...
							if(get_option('erphp_remind')){
								$headers = 'Content-Type: text/html; charset=' . get_option('blog_charset') . "\n";
								wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】订单提醒 - '.$subject, '用户'.get_user_by('id',$order->ice_user_id)->user_login.'消费'.$price.get_option('ice_name_alipay').'购买了'.$subject.get_permalink($postid), $headers);
							}
				...
				addUserMoney($order->ice_user_id, '-'.$total_fee*get_option('ice_proportion_alipay'), '升级VIP');
				...
				if(get_option('erphp_remind')){
					...
					wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】VIP订单提醒 - '.$typeName, '用户'.get_user_by('id',$order->ice_user_id)->user_login.'消费'.$total_fee.'元购买了'.$typeName, $headers);
				}
			}else{
				if(get_option('erphp_remind_recharge')){
					$headers = 'Content-Type: text/html; charset=' . get_option('blog_charset') . "\n";
					wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】充值提醒 - '.$total_fee_old.'元', '用户'.get_user_by('id',$order->ice_user_id)->user_login.'刚刚充值了'.$total_fee_old.'元。', $headers);
				}
			}
		}
	}
...
function erphpdown_dashboard_widget_function() {
	...
	if(current_user_can('administrator')){
		...
		echo '<li style="margin-bottom:10px;float:left;width:50%"><span>今日充值</span>：<a href="'.admin_url('admin.php?page=erphpdown/admin/erphp-chong-list.php').'">'.($today_chong_money?$today_chong_money:'0').' '.get_option('ice_name_alipay').'</a></li>';
		echo '<li style="margin-bottom:10px;float:left;width:50%"><span>昨日充值</span>：<a href="'.admin_url('admin.php?page=erphpdown/admin/erphp-chong-list.php').'">'.($yestoday_chong_money?$yestoday_chong_money:'0').' '.get_option('ice_name_alipay').'</a></li>';
		echo '<li style="margin-bottom:10px;float:left;width:50%"><span>今日销售</span>：<a href="'.admin_url('admin.php?page=erphpdown/admin/erphp-orders-list.php').'">'.($today_order_money?$today_order_money:'0').' '.get_option('ice_name_alipay').'</a></li>';
		echo '<li style="margin-bottom:10px;float:left;width:50%"><span>昨日销售</span>：<a href="'.admin_url('admin.php?page=erphpdown/admin/erphp-orders-list.php').'">'.($yestoday_order_money?$yestoday_order_money:'0').' '.get_option('ice_name_alipay').'</a></li>';
		echo '</ul></div>';
